...
voidSomeRegularFunction(int value);

template\<class T\>voidSomeTemplateFunction(T item);\
};
...
myVector.push\_back(5);

myVector.push\_back(10);

myVector.push\_back(15);
...
We\'ve seen a number of methods used without any explanation. Sadly, sometimes in the real world the only available documentation for code is the code itself, but it doesn\'t have to be that way here. These are the methods we\'ve seen so far with vectors, deques, and lists and a few other useful ones. By no means is this an exhaustive list, however.
  ...
  *size\_type size();*                Returns the number of elements in the container. *size\_type* is an unsigned integer type.
  *void push\_back(const T &val);*    Adds an item to the end of the container\'s list which has the value *val*. Note that this effectively invalidates any existing iterators.
  *void pop\_back();*                 Deletes the last item in the container.
  *void push\_front(const T &val);*   Adds an item to the beginning of the container\'s list which has the value *val*. Note that this effectively invalidates any existing iterators. This method is unavailable to the *vector* class.
  *void pop\_front();*                Deletes the first item in the container. This method is unavailable to the *vector* class.
...
find\_directory(B\_USER\_DIRECTORY, &path);

BDirectory dir(path.Path());

deque\<entry\_ref\> refDeque;

entry\_ref ref;

while (dir.GetNextRef(&ref) == B\_OK)

refDeque.push\_back(ref);

printf(\"Contents of the home folder: %s**\\n**\", path.Path());

for (deque\<entry\_ref\>::iterator i = refDeque.begin();
...
// a pointer, we can access each entry\_ref\'s name
