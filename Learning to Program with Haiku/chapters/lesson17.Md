...
-   *BWindow(BRect frame, const char \*title, window\_type type, uint32 flags, uint32 workspace = B\_CURRENT\_WORKSPACE) *– Create a new window. While there are other types, right now the window types to remember are *B\_TITLED\_WINDOW* and *B\_DOCUMENT\_WINDOW*.
