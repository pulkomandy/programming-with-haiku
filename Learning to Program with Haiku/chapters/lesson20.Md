...
const uint16 BYTES\_PER\_KB = 1024;

const uint32 BYTES\_PER\_MB = 1048576;

const uint64 BYTES\_PER\_GB = 1099511627776ULL;

intListDirectory(const entry\_ref &dirRef);
...
// An entry\_ref is a typedef'ed structure which points to a file, directory,
...
ListDirectory(const entry\_ref &dirRef)
...
// GetNextRef() will return B\_ERROR when the BDirectory has gotten to the
...
while (dir.GetNextEntry(&entry) == B\_OK)

{

char name\[B\_FILE\_NAME\_LENGTH\];
...
if (size \< BYTES\_PER\_KB)

sizeString \<\< size \<\< " bytes";

else if (size \< BYTES\_PER\_MB)

sizeString \<\< (float(size) / float(BYTES\_PER\_KB)) \<\< " KB";

else if (size \< BYTES\_PER\_GB)

sizeString \<\< (float(size) / float(BYTES\_PER\_MB)) \<\< " MB";

else

sizeString \<\< (float(size) / float(BYTES\_PER\_GB)) \<\< " GB";
