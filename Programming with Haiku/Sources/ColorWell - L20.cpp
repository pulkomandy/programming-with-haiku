...
ColorWell::MouseDown(BPoint pt)
{
	...
	// BBitmap constructor allows for BBitmaps which can accept and be drawn upon
	// by BViews.
	BRect r(0, 0, 15, 15);
	...
	msg.AddData("RGBColor", B_RGB_COLOR_TYPE, &fColor, sizeof(rgb_color));
...
ColorWell::MessageReceived(BMessage *msg)
{
	...
	if (msg->WasDropped())
	{
		rgb_color *c;
		ssize_t size;
		if (msg->FindData("RGBColor", B_RGB_COLOR_TYPE, (const void **)&c, &size) == B_OK)
		{
			SetValue(*c);
			return;
		}
